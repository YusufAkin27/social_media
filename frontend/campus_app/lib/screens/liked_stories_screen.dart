...
class _LikedStoriesScreenState extends State<LikedStoriesScreen> {
  ...
  Future<void> _fetchLikedStories() async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse('http://localhost:8080/v1/api/stories/liked?page=$_page&pageSize=$_pageSize'),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _errorMessage = 'Beğenilen hikayeler alınamadı: ${response.statusCode}';
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  Future<void> _unlikeStory(String storyId) async {
    try {
      ...
      final response = await http.post(
        Uri.parse('http://localhost:8080/v1/api/stories/$storyId/unlike'),
        headers: {
          'Authorization': 'Bearer $accessToken',
        },
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colors.black,
        title: const Text('Beğenilen Hikayeler', style: TextStyle(color: Colors.white)),
              ...
              itemBuilder: (context) => [
                const PopupMenuItem(
                  ...
                  child: ListTile(
                    ...
                    title: Text('Beğenmekten Vazgeç', style: TextStyle(color: Colors.white)),
                ...
                const PopupMenuItem(
                  ...
                  child: ListTile(
                    ...
                    title: Text('Raporla', style: TextStyle(color: Colors.white)),
                    contentPadding: EdgeInsets.zero,
                    dense: true,
                  ),
                ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              _errorMessage ?? 'Beğenilen hikayeler yüklenirken bir hata oluştu.',
