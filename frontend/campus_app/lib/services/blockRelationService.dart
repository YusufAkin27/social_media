...
class BlockRelationService {
  ...
  Future<DataResponseMessage<List<BlockUserDTO>>> getBlockedUsers(String token, int page) async {
    ...
    return DataResponseMessage<List<BlockUserDTO>>.fromJson(
      response.data,
      (data) => (data as List).map((item) => BlockUserDTO.fromJson(item)).toList(),
    );
  }
  ...
  Future<DataResponseMessage<List<DateTime>>> getBlockHistory(String token, int userId) async {
    final response = await _dio.get(
  ...
  Future<DataResponseMessage<BlockUserDTO>> getUserDetails(String token, int userId) async {
    final response = await _dio.get(
  ...
  Future<Response> addWithReasonAndComment(String token, int userId, String reason) async {
    final response = await _dio.post(
      '/block-relations/block/$userId',
      data: {'reason': reason},
      options: Options(headers: {'Authorization': 'Bearer $token'}),
