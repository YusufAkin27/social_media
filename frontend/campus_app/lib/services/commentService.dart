...
class CommentService {
  ...
  Future<Response> addCommentToStory(String token, String storyId, String content) async {
  ...
  Future<Response> addCommentToPost(String token, String postId, String content) async {
    try {
      if (content.isEmpty) {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception("Failed to add comment: ${response.statusCode} - ${response.data}");
      }
    } catch (e) {
  ...
  Future<Response> getStoryComments(String token, String storyId, int page) async {
    final response = await _dio.get(
      '/comments/story/$storyId',
      queryParameters: {'page': page},
      options: Options(headers: {'Authorization': 'Bearer $token'}),
  ...
  Future<Response> getPostComments(String token, String postId, int page, {int limit = 10}) async {
    final response = await _dio.get(
  ...
  Future<Response> searchUserInStoryComments(String token, String storyId, String username) async {
    final response = await _dio.get(
  ...
  Future<Response> searchUserInPostComments(String token, String postId, String username) async {
    final response = await _dio.get(
