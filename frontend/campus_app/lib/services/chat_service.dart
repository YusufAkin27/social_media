...
class ChatService {
  ...
  ChatService(this._dio) {
    _dio.options.baseUrl = 'http://localhost:8080/v1/api/privateChat'; // Base URL ayarlandı
  }
  ...
  // Mesaj gönder
  Future<DataResponseMessage<MessageDTO>> sendPrivateMessage(String token, SendMessageRequest request) async {
    final response = await _dio.post(
      '/send',
      data: request.toJson(),
      options: Options(headers: {'Authorization': 'Bearer $token'}),
  ...
  Future<DataResponseMessage<List<PrivateChatDTO>>> getChats(String token) async {
    ...
    return DataResponseMessage<List<PrivateChatDTO>>.fromJson(
      response.data,
      (data) => (data as List).map((item) => PrivateChatDTO.fromJson(item as Map<String, dynamic>)).toList(),
    );
  }

  // Belirli bir sohbetin mesajlarını getir
  Future<DataResponseMessage<List<MessageDTO>>> getMessages(String token, String chatId) async {
    final response = await _dio.get(
      '/getMessages/$chatId',
      options: Options(headers: {'Authorization': 'Bearer $token'}),
    );
  ...
  // Mesajı düzenle
  Future<ResponseMessage> editMessage(String token, EditMessageRequest request) async {
  ...
  // Mesajı sil
  Future<ResponseMessage> deleteMessage(String token, DeleteMessageRequest request) async {
  ...
  Future<DataResponseMessage<List<String>>> getReadReceipts(String token, String messageId) async {
    final response = await _dio.get(
  ...
  Future<DataResponseMessage<Map<String, Object>>> getUserStatus(String token, String username) async {
    final response = await _dio.get(
      '/userStatus/$username',
      options: Options(headers: {'Authorization': 'Bearer $token'}),
    );
  ...
  Future<ResponseMessage> updateMessageStatus(String token, UpdateMessageStatusRequest request) async {
    final response = await _dio.put(
      '/updateMessageStatus',
      data: request.toJson(),
      options: Options(headers: {'Authorization': 'Bearer $token'}),
