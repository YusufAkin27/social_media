...
class LogService {
  ...
  Future<DataResponseMessage<List<LogsDTO>>> getLogs(String token) async {
    ...
    return DataResponseMessage<List<LogsDTO>>.fromJson(
      response.data,
      (data) => (data as List).map((item) => LogsDTO.fromJson(item as Map<String, dynamic>)).toList(),
    );
  }

  // Belirli bir logun detaylarını alma
  Future<DataResponseMessage<LogsDTO>> getLogDetails(String token, String logId) async {
    final response = await _dio.get(
      '/logs/$logId/details',
      options: Options(headers: {'Authorization': 'Bearer $token'}),
    );
  ...
  Future<DataResponseMessage<List<LogsDTO>>> getLogsPaginated(String token, int page) async {
    final response = await _dio.get(
      '/logs',
